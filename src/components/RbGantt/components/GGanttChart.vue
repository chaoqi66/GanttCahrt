<template>
  <div >
    ...
    <g-gantt-bar-tooltip v-if="tooltipBar && tooltipBar.ganttBarConfig && !tooltipBar.ganttBarConfig.virtual" :model-value="showTooltip || isDragging" :bar="tooltipBar">
      <template #default>
...
provide('virtualBarData', virtualBar)
...
const updateVirtualData = (data) => {
  ...
  eventBus.emit("check-is-exist-bar", virtualBar.value);
}
const closeVirtualBar = () => {
  virtualBar.value = {}
}
defineExpose({
  updateVirtualData,
...
onMounted(() => {
  eventBus.on("updateVirtualData", (data) => {
    virtualBar.value = data
  });
})
watch(
  () => virtualBar.value,
